...
const FacultyReportsAndAttendance = ({ user, onToast = () => {} }) => {
  ...
  return (
    <div className="space-y-6 text-left">
      {/* Module Banner Header (Clean White / Soft Slate Background) */}
      <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 bg-white p-6 rounded-3xl border border-slate-200/90 shadow-sm">
        <div className="space-y-1.5 max-w-2xl">
          <div className="flex items-center gap-2 flex-wrap">
            <span className="px-3 py-1 rounded-full bg-blue-50 text-blue-700 font-extrabold text-[11px] uppercase tracking-wider border border-blue-200 flex items-center gap-1.5">
              <FolderCheck size={14} />
        ...
        <div className="flex items-center gap-2.5 flex-wrap self-start lg:self-center">
          <button
            ...
            className="px-4 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold text-xs flex items-center gap-2 cursor-pointer shadow-sm transition-all active:scale-95"
          >
          ...
          <button
            ...
            className="px-4 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-extrabold text-xs flex items-center gap-2 cursor-pointer shadow-sm transition-all active:scale-95"
          >
            <Download size={15} />
      ...
      {/* TOP STATS CARDS (4 KPIs Required - White Background) */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {/* 1. Total Events */}
        <div className="p-5 rounded-2xl bg-white border border-slate-200/90 shadow-xs space-y-1 hover:border-blue-300 hover:shadow-sm transition-all">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-extrabold uppercase text-slate-500 tracking-wider">Total Events</span>
            <div className="w-8 h-8 rounded-xl bg-blue-50 text-blue-600 flex items-center justify-center border border-blue-200">
              <Calendar size={16} />
            </div>
          </div>
          <div className="flex items-baseline gap-2">
            <span className="text-3xl font-black text-slate-900">{stats.totalEvents}</span>
            <span className="text-xs text-blue-600 font-bold">Organized</span>
          </div>
          <p className="text-[11px] text-slate-500">Across all academic semesters</p>
        </div>

        {/* 2. Reports Available */}
        <div className="p-5 rounded-2xl bg-white border border-slate-200/90 shadow-xs space-y-1 hover:border-amber-300 hover:shadow-sm transition-all">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-extrabold uppercase text-slate-500 tracking-wider">Reports Available</span>
            <div className="w-8 h-8 rounded-xl bg-amber-50 text-amber-600 flex items-center justify-center border border-amber-200">
              <FileText size={16} />
            </div>
          </div>
          <div className="flex items-baseline gap-2">
            <span className="text-3xl font-black text-amber-600">{stats.reportsAvailable}</span>
            <span className="text-xs text-slate-500 font-bold">/ {stats.totalEvents} Events</span>
          </div>
          <p className="text-[11px] text-slate-500">{stats.verifiedReports} officially approved & verified</p>
        </div>

        {/* 3. Total Attendance */}
        <div className="p-5 rounded-2xl bg-white border border-slate-200/90 shadow-xs space-y-1 hover:border-emerald-300 hover:shadow-sm transition-all">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-extrabold uppercase text-slate-500 tracking-wider">Total Attendance</span>
          ...
          <div className="flex items-baseline gap-2">
            <span className="text-3xl font-black text-emerald-600">{stats.totalAttendance.toLocaleString()}</span>
            <span className="text-xs text-emerald-600 font-bold">Students</span>
          </div>
          <p className="text-[11px] text-slate-500">100% verified via QR check-ins</p>
        </div>

        {/* 4. Total Clubs */}
        <div className="p-5 rounded-2xl bg-white border border-slate-200/90 shadow-xs space-y-1 hover:border-purple-300 hover:shadow-sm transition-all">
          <div className="flex items-center justify-between">
            <span className="text-[11px] font-extrabold uppercase text-slate-500 tracking-wider">Total Clubs</span>
            <div className="w-8 h-8 rounded-xl bg-purple-50 text-purple-600 flex items-center justify-center border border-purple-200">
              <Building2 size={16} />
            </div>
          </div>
          <div className="flex items-baseline gap-2">
            <span className="text-3xl font-black text-purple-600">{stats.totalClubs}</span>
            <span className="text-xs text-purple-600 font-bold">Monitored</span>
          </div>
          <p className="text-[11px] text-slate-500">Tech, Cultural, NCC, NSS & Sports</p>
        </div>
      </div>

      {/* WORKING FILTERS BAR (Clean White / Soft Slate) */}
      <div className="bg-white p-5 rounded-3xl border border-slate-200/90 space-y-4 shadow-xs">
        <div className="flex items-center justify-between gap-2 border-b border-slate-100 pb-3">
          <div className="flex items-center gap-2">
            <Filter size={16} className="text-blue-600" />
            <h3 className="text-sm font-extrabold text-slate-900">Interactive Filters & Search</h3>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3">
          ...
          <div className="space-y-1 lg:col-span-2">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-8 pr-3 py-2 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 text-xs placeholder-slate-400 outline-none focus:bg-white focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all"
              />
            </div>
          </div>
          ...
          <div className="space-y-1">
            <label className="text-[10px] uppercase font-bold text-slate-500">Club</label>
            <select
              value={selectedClub}
              onChange={(e) => setSelectedClub(e.target.value)}
              className="w-full px-3 py-2 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 text-xs outline-none focus:bg-white focus:border-blue-500 cursor-pointer font-medium"
            >
          ...
          <div className="space-y-1">
            <label className="text-[10px] uppercase font-bold text-slate-500">Event Category</label>
            <select
          ...
          <div className="space-y-1">
            <label className="text-[10px] uppercase font-bold text-slate-500">Academic Year</label>
            <select
      ...
      {/* EVENTS TABLE & CARDS SECTION (Clean White Background) */}
      <div className="bg-white rounded-3xl border border-slate-200/90 overflow-hidden shadow-xs space-y-4 p-5">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 pb-3">
          ...
          <div className="flex items-center gap-2">
            <button
              onClick={handleExportAllAttendance}
              className="px-3 py-1.5 rounded-xl bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-200 font-bold text-xs flex items-center gap-1.5 cursor-pointer transition-all active:scale-95"
            >
            ...
            <button
              onClick={handleExportInstitutionalPDF}
              className="px-3 py-1.5 rounded-xl bg-blue-50 hover:bg-blue-100 text-blue-700 border border-blue-200 font-bold text-xs flex items-center gap-1.5 cursor-pointer transition-all active:scale-95"
            >
              <FileText size={13} />
        ...
        {filteredEvents.length === 0 ? (
          <div className="p-12 text-center rounded-2xl bg-slate-50 border border-slate-200 space-y-3">
            ...
            <h4 className="text-base font-bold text-slate-800">No campus events match your filters</h4>
            <p className="text-xs text-slate-500 max-w-md mx-auto">
              Try adjusting your search terms, club selection, branch filter, or academic year.
            </p>
            <button
              onClick={handleResetFilters}
              className="px-4 py-2 rounded-xl bg-blue-600 text-white font-bold text-xs cursor-pointer hover:bg-blue-700 transition-colors shadow-xs"
            >
              Clear All Filters
            </button>
          </div>
          ...
          <div className="space-y-4">
            {filteredEvents.map((event) => {
              ...
              return (
                <div
                  key={event.id}
                  className="p-5 rounded-2xl bg-white border border-slate-200/90 hover:border-blue-300 hover:shadow-sm transition-all space-y-4 shadow-xs"
                >
                  ...
                  <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-4">
                    ...
                    <div className="space-y-2 flex-1 min-w-0">
                      ...
                      {/* Event Name */}
                      <h4 className="text-lg font-black text-slate-900 leading-snug hover:text-blue-600 transition-colors">
                        {event.title}
                      </h4>
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs text-slate-600 pt-1">
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          <span>
                            <strong className="text-emerald-700 font-bold">{event.totalAttendance}</strong> Participants
                          </span>
                        </div>

                        <div className="flex items-center gap-2">
                          <span className="px-2 py-0.5 rounded-md bg-slate-100 text-slate-700 text-[11px] font-bold border border-slate-200 flex items-center gap-1">
                            <FileCheck size={11} className="text-blue-600" />
                            <span>{event.documentsCount || 4} Docs</span>
                          </span>

                          <span className="px-2 py-0.5 rounded-md bg-slate-100 text-slate-700 text-[11px] font-bold border border-slate-200 flex items-center gap-1">
                            <ImageIcon size={11} className="text-pink-600" />
                    ...
                    <div className="lg:w-72 p-3 rounded-xl bg-slate-50 border border-slate-200/80 shrink-0 space-y-1.5 text-xs">
                      <div className="flex items-center justify-between text-[10px] font-bold uppercase text-slate-500">
                        <span>Branch Turnout</span>
                        <span className="text-emerald-700 font-mono font-bold">{event.totalAttendance} Total</span>
                      </div>
                      ...
                      <div className="flex items-center gap-1 flex-wrap">
                        {event.branchBreakdown?.slice(0, 5).map(b => (
                          <span 
                            key={b.branch} 
                            className="px-2 py-0.5 rounded-md bg-white text-[10px] font-bold text-slate-700 border border-slate-200 shadow-2xs"
                            title={`${b.count} students (${b.percentage}%)`}
                          >
                            <strong className="text-slate-900">{b.branch}:</strong> <span className="text-emerald-600 font-mono">{b.count}</span>
                          </span>
                        ))}
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-3 border-t border-slate-100">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <button
                        ...
                        className="px-3.5 py-2 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-extrabold text-xs flex items-center gap-1.5 cursor-pointer shadow-xs transition-all active:scale-95"
                      >
                        <FileText size={14} />
                      ...
                      <button
                        ...
                        className="px-3.5 py-2 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold text-xs flex items-center gap-1.5 cursor-pointer shadow-xs transition-all active:scale-95"
                      >
                        <Users size={14} />
                      ...
                      <button
                        ...
                        className="px-3.5 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-extrabold text-xs flex items-center gap-1.5 cursor-pointer shadow-xs transition-all active:scale-95"
                      >
                        <FileCheck size={14} />
                    ...
                    <div className="relative">
                      <button
                        ...
                        className="px-3.5 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 font-extrabold text-xs flex items-center gap-2 cursor-pointer border border-slate-200 transition-colors"
                      >
                      ...
                      <AnimatePresence>
                        {isDownloadOpen && (
                          <motion.div
                            ...
                            className="absolute right-0 bottom-full mb-2 w-64 bg-white border border-slate-200 rounded-2xl shadow-xl p-1.5 z-30 space-y-1"
                          >
                            <button
                              onClick={() => handleDownloadPDFReport(event)}
                              className="w-full px-3 py-2 rounded-xl text-xs font-semibold text-slate-700 hover:text-blue-700 hover:bg-blue-50 flex items-center gap-2 cursor-pointer transition-colors text-left"
                            >
                            ...
                            <button
                              ...
                              className="w-full px-3 py-2 rounded-xl text-xs font-semibold text-slate-700 hover:text-emerald-700 hover:bg-emerald-50 flex items-center gap-2 cursor-pointer transition-colors text-left"
                            >
                            ...
                            <button
                              ...
                              className="w-full px-3 py-2 rounded-xl text-xs font-semibold text-slate-700 hover:text-purple-700 hover:bg-purple-50 flex items-center gap-2 cursor-pointer transition-colors text-left"
                            >
